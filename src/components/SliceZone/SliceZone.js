    ...
    const slice = allSlices.map((slice) => {
      switch (slice.slice_type) {
        ...
        case 'one_column_text':
        	return <OneColumnText key={slice.id} input={slice} />
        case 'two_column_with_image':
        	return <TwoColumnWithImage key={slice.id} input={slice} />
		    case 'two_column_with_slider':
			    return <TwoColumnWithSlider key={slice.id} input={slice} />
		    case 'box_grid':
			    return <BoxGrid key={slice.id} input={slice} />
		    case 'form_body':
			    return <FormBody key={slice.id} input={slice} />
        default:
        	return null
      }
    })
